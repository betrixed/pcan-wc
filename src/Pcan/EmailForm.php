...
class EmailForm extends Controller {
    ...
    private function render($isSub = false) {
        ...
        $this->captchaView($m);
        $this->xcheckView($m);
        $isAjax = $this->f3->get('AJAX') ? true : false;
        $view->assets(['ajax-form-captcha']);
      /*  if ($m->google['enabled']) {
            $view->assets(['ajax-form-captcha']);
        }
        else {
              $view->assets(['register-js']);
        }*/
        
        if ($isAjax) {
            $view->layout = null;
            $view->content = 'form/email_form';
        } else  {
    ...
    public function post($f3, $args) {
        ...
        try {
            ...
            $model->rec = $rec;
            $model->link = UserSession::getURL($f3);
            $textMsg = static::renderView($model, 'form/mail_text.phtml');
            $htmlMsg = static::renderView($model, 'form/mail_html.phtml');
